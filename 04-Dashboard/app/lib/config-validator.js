...
function validateAllConfigs() {
    ...
    if (!profileResult.errors.length && fs.existsSync(profileFile)) {
        ...
        if (!profile.professional_context)
            profileResult.warnings.push('Missing professional_context — context injection will be limited');
        if (!profile.output_preferences)
            profileResult.warnings.push('Missing output_preferences — AI output style not customized');
    }
    results.push(profileResult);
    // Check .env
    const envFile = path.resolve(__dirname, '..', '.env');
    const envResult = { file: '.env', valid: true, errors: [], warnings: [] };
    if (!fs.existsSync(envFile)) {
        ...
        if (!env.includes('OPENAI_API_KEY') || env.includes('your_key_here'))
            envResult.warnings.push('OPENAI_API_KEY may not be configured');
        if (!env.includes('PERPLEXITY_API_KEY'))
            envResult.warnings.push('PERPLEXITY_API_KEY not set — web search unavailable');
        if (!env.includes('GEMINI_API_KEY'))
            envResult.warnings.push('GEMINI_API_KEY not set — strategy engine limited');
    }
...
function validateJsonFile(filePath, requiredKeys) {
    ...
    const result = { file: name, valid: true, errors: [], warnings: [] };
    ...
    try {
        ...
        for (const key of requiredKeys) {
            if (!(key in data))
                result.warnings.push(`Missing key: ${key}`);
        }
    }
    catch (e) {
        result.valid = false;
        result.errors.push(`Invalid JSON: ${e.message?.slice(0, 80)}`);
    }
    return result;
}
