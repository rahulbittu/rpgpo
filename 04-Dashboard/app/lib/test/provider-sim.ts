...
export function createProviderSim(opts: ProviderSimOptions): {
  callProvider(provider: string, schemaId: string, prompt: string): Promise<{ text: string; tokensIn: number; tokensOut: number; latencyMs: number }>;
  getCallCount(): number;
  reset(): void;
} {
  const state: SimState = { callCount: 0, rng: seededRng(opts.seed) };
  const behavior = opts.behavior || {};

  async function callProvider(provider: string, schemaId: string, prompt: string): Promise<{ text: string; tokensIn: number; tokensOut: number; latencyMs: number }> {
    state.callCount++;
...
function generateContractOutput(schemaId: string, rng: () => number): any {
  switch (schemaId) {
