...
async function runScenario(scenario) {
    ...
    try {
        ...
        try {
            fs.unlinkSync(path.resolve(__dirname, '..', '..', '..', 'state', 'workflows.json'));
        }
        catch { /* */ }
        try {
            fs.rmSync(path.resolve(__dirname, '..', '..', '..', 'state', 'evidence', 'orchestrator'), { recursive: true });
        }
        catch { /* */ }
    }
    catch (e) {
        result.passed = false;
        result.errors.push(e.message?.slice(0, 200) || 'Unknown error');
    }
    result.duration = Date.now() - start;
    return result;
}
function getSchemaForEngine(engine) {
    const schemas = {
