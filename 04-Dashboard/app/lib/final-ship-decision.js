...
exports.getWorkflowClosure = getWorkflowClosure;
exports.computeDecision = computeDecision;
function uid() { return 'fsd_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
/** Compute final workflow closure report */
function getWorkflowClosure() {
    const closedInThisPass = [];
    try {
...
function computeDecision() {
    ...
    try {
        ...
        if (r.open > 0)
            remainingGaps.push(`${r.open} open blockers`);
        if (r.partially_closed > 0)
            remainingGaps.push(`${r.partially_closed} partially closed blockers`);
    }
    catch {
        evidence.push({ dimension: 'Blocker reconciliation', score: 0, max: 7, status: 'fail', detail: 'Module not available' });
    }
    // 2. Workflow closure
    let workflowScore = 0;
    try {
        ...
        evidence.push({ dimension: 'Workflow closure', score: 0, max: 13, status: 'fail', detail: 'Module not available' });
    }
    // 3. Middleware truth
    ...
    try {
        ...
        evidence.push({ dimension: 'Middleware truth', score: 0, max: 6, status: 'fail', detail: 'Module not available' });
    }
    ...
    try {
        ...
        if (run) {
            ...
            if (run.passed < run.total)
                remainingGaps.push(`${run.total - run.passed} HTTP validation case(s) failed`);
        }
        else {
        ...
        evidence.push({ dimension: 'HTTP middleware validation', score: 0, max: 8, status: 'fail', detail: 'Module not available' });
    }
    ...
    try {
        ...
        evidence.push({ dimension: 'Operator acceptance', score: 0, max: 13, status: 'fail', detail: 'Module not available' });
    }
    // 6. Security posture
    ...
    try {
        ...
        evidence.push({ dimension: 'Security posture', score: 5, max: 10, status: 'conditional', detail: 'Security module not available' });
    }
    ...
    try {
        ...
        evidence.push({ dimension: 'Measured reliability', score: 5, max: 10, status: 'conditional', detail: 'Reliability module not available' });
    }
    // Compute overall score — weighted
    const overallScore = Math.round(blockerScore * 0.20 +
        workflowScore * 0.15 +
...
//# sourceMappingURL=final-ship-decision.js.map