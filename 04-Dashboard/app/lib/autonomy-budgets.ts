...
import type {
  Domain, Lane, AutonomyBudget, RiskLevel, MissionStatementLevel,
} from './types';
...
function uid(): string { return 'ab_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }

function readBudgets(): AutonomyBudget[] {
  try { return fs.existsSync(BUDGETS_FILE) ? JSON.parse(fs.readFileSync(BUDGETS_FILE, 'utf-8')) : []; } catch { return []; }
}

function writeBudgets(b: AutonomyBudget[]): void {
...
function defaultBudget(lane: Lane): Omit<AutonomyBudget, 'budget_id' | 'scope_level' | 'scope_id' | 'created_at' | 'updated_at'> {
  switch (lane) {
    case 'dev': return {
      lane, enabled: true,
...
export function getAllBudgets(): AutonomyBudget[] {
  return readBudgets();
}

export function getBudgetsForDomain(domain: Domain): AutonomyBudget[] {
  return readBudgets().filter(b =>
    (b.scope_level === 'engine' && b.scope_id === domain) || b.scope_level === 'global'
  );
}

export function getBudgetsForProject(projectId: string): AutonomyBudget[] {
  return readBudgets().filter(b =>
    (b.scope_level === 'project' && b.scope_id === projectId) || b.scope_level === 'global'
  );
}
...
export function createBudget(opts: {
  ...
  scope_id: string;
  lane: Lane;
} & Partial<AutonomyBudget>): AutonomyBudget {
...
export function toggleBudget(budgetId: string): AutonomyBudget | null {
  const budgets = readBudgets();
