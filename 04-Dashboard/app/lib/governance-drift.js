...
function detectDrift(scopeLevel, scopeId, domain) {
    const signals = [];
    ...
    try {
        ...
        const blocked = decisions.filter((d) => d.result === 'blocked');
        if (blocked.length >= 2) {
    ...
    try {
        ...
        const runs = readJson(rpFile, []);
        const antiPatterns = runs.flatMap((r) => r.anti_patterns || []);
        const providerMismatches = antiPatterns.filter((p) => p.toLowerCase().includes('provider mismatch'));
...
function getReports(scopeLevel, scopeId) {
    const all = readJson(REPORTS_FILE, []);
    if (scopeLevel && scopeId)
        return all.filter(r => r.scope_level === scopeLevel && r.scope_id === scopeId);
    return all;
}
module.exports = { detectDrift, getReports };
//# sourceMappingURL=governance-drift.js.map