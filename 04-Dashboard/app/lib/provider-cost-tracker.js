...
exports.getProviderCostBreakdown = getProviderCostBreakdown;
// GPO Provider Cost Tracker — Per-provider cost breakdown with trends
function getProviderCostBreakdown(days = 7) {
    const result = {};
    try {
        const fs = require('fs');
        ...
        for (const c of costs) {
            ...
            if (!result[p])
                result[p] = { totalUsd: 0, calls: 0, avgPerCall: 0, trend: 'stable' };
        ...
        for (const [p, data] of Object.entries(result)) {
            ...
            const firstHalf = costs.filter((c) => { const ts = new Date(c.timestamp || c.created_at || '').getTime(); return ts > cutoff && ts < midpoint && (c.provider || 'unknown') === p; }).reduce((s, c) => s + (c.cost || 0), 0);
            const secondHalf = costs.filter((c) => { const ts = new Date(c.timestamp || c.created_at || '').getTime(); return ts >= midpoint && (c.provider || 'unknown') === p; }).reduce((s, c) => s + (c.cost || 0), 0);
