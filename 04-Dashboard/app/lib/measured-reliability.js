...
function uid() { return 'mr_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
/** Compute measured reliability from real telemetry */
function computeReliability(scope = 'platform') {
    const metrics = [];
    const regressions = [];
    ...
    metrics.push({ name: 'execution_success_rate', value: successRate, unit: '%', measured: total > 0, window: '7d' }, { name: 'execution_failure_rate', value: failureRate, unit: '%', measured: total > 0, window: '7d' }, { name: 'runtime_block_rate', value: blockRate, unit: '%', measured: total > 0, window: '7d' }, { name: 'avg_action_latency', value: avgDuration, unit: 'ms', measured: durations.length > 0, window: '7d' }, { name: 'total_events', value: total, unit: 'events', measured: true, window: '7d' });
    ...
    if (successRate < 90)
        regressions.push({ metric: 'execution_success_rate', previous: 100, current: successRate, delta: successRate - 100 });
    if (blockRate > 10)
        regressions.push({ metric: 'runtime_block_rate', previous: 0, current: blockRate, delta: blockRate });
    let health = 'healthy';
    if (regressions.length >= 2 || successRate < 70)
        health = 'critical';
    else if (regressions.length >= 1 || successRate < 85)
        health = 'degraded';
...
//# sourceMappingURL=measured-reliability.js.map