...
import type { NetworkHTTPValidationRun, NetworkHTTPCase, NetworkHTTPResult, ValidationHarnessState } from './types';
...
function uid(): string { return 'nhv_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
...
export function getCases(): NetworkHTTPCase[] {
  return [
...
function probeServer(): Promise<ValidationHarnessState> {
  return new Promise((resolve) => {
    const start = Date.now();
    const req = http.get(`${BASE_URL}/api/middleware-truth`, { timeout: 3000 }, (res: import('http').IncomingMessage) => {
      let body = '';
...
function httpGet(route: string): Promise<{ status: number; body: string }> {
  return new Promise((resolve) => {
    const req = http.get(`${BASE_URL}${route}`, { timeout: 5000 }, (res: import('http').IncomingMessage) => {
      let body = '';
      res.on('data', (c: Buffer) => { body += c; });
...
export async function runValidation(): Promise<NetworkHTTPValidationRun> {
  const harness = await probeServer();
  const cases = getCases();
  const results: NetworkHTTPResult[] = [];
  ...
  const run: NetworkHTTPValidationRun = {
  ...
  const runs = readJson<NetworkHTTPValidationRun[]>(RUNS_FILE, []);
  runs.unshift(run);
...
async function validateCaseNetwork(c: NetworkHTTPCase): Promise<NetworkHTTPResult> {
...
function validateCaseFunction(c: NetworkHTTPCase): NetworkHTTPResult {
  // Delegate to Part 52 http-middleware-validation for function-level check
  try {
    const hmv = require('./http-middleware-validation') as { runValidation(): { results: Array<{ case_id: string; passed: boolean; actual_effect: string; middleware_executed: string[]; detail: string }> } };
...
function validateCaseDirect(c: NetworkHTTPCase): NetworkHTTPResult {
  ...
  if (c.case_id === 'net_entitle_deny' || c.case_id === 'net_entitle_allow') {
    try {
      ...
      detail = d.reason; mw.push('api-entitlement-enforcement');
    ...
    try {
      ...
      detail = r.reason; mw.push('boundary-enforcement'); passed = actualEffect === c.expected_effect;
    } catch (e) { detail = String(e); }
    ...
    try {
      ...
      detail = r.reason; mw.push('middleware-enforcement'); passed = actualEffect === c.expected_effect;
    } catch (e) { detail = String(e); }
    ...
    try {
      ...
      actualEffect = r.outcome as string; detail = r.reason; mw.push('data-boundaries');
      passed = actualEffect === c.expected_effect;
    } catch (e) { detail = String(e); }
    ...
    try {
      ...
      detail = r.reason; mw.push('extension-permission-enforcement'); passed = actualEffect === c.expected_effect;
    } catch (e) { detail = String(e); }
    ...
    try {
      ...
      detail = r.detail; mw.push('release-provider-gating'); passed = actualEffect === c.expected_effect;
    } catch (e) { detail = String(e); }
...
function recordNetworkEvidence(c: NetworkHTTPCase, status: number, effect: string, passed: boolean): string {
  try {
...
/** Get latest run */
export function getLatestRun(): NetworkHTTPValidationRun | null {
  const runs = readJson<NetworkHTTPValidationRun[]>(RUNS_FILE, []);
