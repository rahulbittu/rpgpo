...
import type { GoLiveClosureReport } from './types';

function uid(): string { return 'glc_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }

/** Generate the go-live closure report by aggregating all readiness signals */
export function getClosureReport(): GoLiveClosureReport {
  const blockers: GoLiveClosureReport['blockers'] = [];

  // Ship blockers from Part 49
  try {
    const sbc = require('./ship-blocker-closure') as { getSummary(): { blockers: Array<{ name: string; status: string; evidence: string }> } };
  ...
  try {
    const me = require('./middleware-enforcement') as { getCoverageReport(): Array<{ area: string; status: string }> };
  ...
  // Operator acceptance
  try {
    const oa = require('./operator-acceptance') as { runAcceptance(): { usable: number; partial: number; broken: number } };
  ...
  try {
    const prc = require('./production-readiness-closure') as { getShipDecision(): { decision: string; readiness_score: number } };
  ...
  const remaining_risk: string[] = [];
  for (const b of blockers) {
    if (b.status !== 'closed') remaining_risk.push(`${b.name}: ${b.evidence}`);
  }

  let recommendation: GoLiveClosureReport['recommendation'] = 'no_go';
  if (blocked === 0 && partial === 0) recommendation = 'go';
  else if (blocked === 0 && partial <= 2) recommendation = 'conditional_go';
