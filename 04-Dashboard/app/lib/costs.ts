...
const COSTS_FILE: string = path.resolve(__dirname, '..', '..', 'state', 'costs.json');
const SETTINGS_FILE: string = path.resolve(__dirname, '..', '..', 'state', 'cost-settings.json');
...
function estimateCost(model: string | undefined, inputTokens: number | undefined, outputTokens: number | undefined): number {
  if (!model) return 0;
...
function getCosts(opts: GetCostsOpts = {}): CostEntry[] {
...
function getSummary(): CostSummary {
  ...
  const sumCost = (arr: CostEntry[]): number => arr.reduce((s, e) => s + toNum(e.cost), 0);
  const sumTokens = (arr: CostEntry[]): number => arr.reduce((s, e) => s + toNum(e.totalTokens), 0);
  ...
  let lastBoardRun: (CostAggregate & { id: string }) | null = null;
...
function getSettings(): BudgetSettings {
...
function updateSettings(updates: Partial<BudgetSettings>): BudgetSettings {
  const current = getSettings();
  const merged: BudgetSettings = { ...current, ...updates };
  fs.writeFileSync(SETTINGS_FILE, JSON.stringify(merged, null, 2));
  return merged;
}

function checkBudget(provider: string): BudgetCheck {
