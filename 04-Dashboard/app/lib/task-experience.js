...
exports.getTaskExperience = getTaskExperience;
exports.getAllExperiences = getAllExperiences;
exports.getShippableSurfaces = getShippableSurfaces;
const fs = require('fs');
...
function getTaskExperience(taskId) {
    ...
    if (isDone)
        stage = 'result';
    else if (taskSubs.some(st => st.status === 'running' || st.status === 'builder_running'))
        stage = 'execution';
    else if (taskSubs.some(st => st.status === 'waiting_approval' || st.status === 'waiting_human'))
        stage = 'approvals';
    else if (hasPlan)
        stage = 'plan';
    else if (hasBoardInterp)
        stage = 'deliberation';
    return {
...
//# sourceMappingURL=task-experience.js.map