...
export interface Suggestion {
  ...
  urgency: string;
  confidence: number;
  source: 'time_of_day' | 'priority' | 'pattern' | 'follow_up' | 'recurring';
}

/**
 * Generate smart suggestions based on current context.
 */
export function getSuggestions(limit: number = 5): Suggestion[] {
  ...
  try {
    ...
    const recentDone = intake.getAllTasks().filter((t: any) => t.status === 'done')
