...
function uid() { return 'ri_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
...
function getSnapshots() {
    ...
    for (const sub of SUBSYSTEMS) {
        ...
        let status = 'healthy';
        if (subIncidents.length >= 3)
            status = 'critical';
        else if (subIncidents.length >= 2)
            status = 'degraded';
        else if (subIncidents.length >= 1)
            status = 'watch';
...
function getIncidents() { return readJson(INCIDENTS_FILE, []); }
...
//# sourceMappingURL=reliability-governance.js.map