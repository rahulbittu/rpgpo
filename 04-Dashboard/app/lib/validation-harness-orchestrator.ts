...
import type { ValidationHarnessExecution, ValidationEnvironmentState } from './types';
...
function uid(): string { return 'vho_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
...
export function detectEnvironment(): Promise<ValidationEnvironmentState> {
...
export async function runHarness(): Promise<ValidationHarnessExecution> {
  const env = await detectEnvironment();
  const phases: ValidationHarnessExecution['phases'] = [];

  // Phase 1: Clean state
  try {
    const csv = require('./clean-state-verification') as { clearValidationState(): { cleared: string[] }; verify(): { clean: boolean } };
  ...
  } catch { phases.push({ phase: 'Clean state', status: 'skipped', detail: 'Module not available' }); }

  // Phase 2: Protected path validation (Part 51)
  try {
    const ppv = require('./protected-path-validation') as { validateAll(): Array<{ overall: string }> };
  ...
  } catch { phases.push({ phase: 'Protected paths', status: 'skipped', detail: 'Module not available' }); }

  // Phase 3: HTTP middleware validation (Part 52)
  try {
    const hmv = require('./http-middleware-validation') as { runValidation(): { passed: number; total: number } };
  ...
  } catch { phases.push({ phase: 'HTTP middleware', status: 'skipped', detail: 'Module not available' }); }

  // Phase 4: Live server proof (Part 54)
  let proofRun = null;
  try {
    const lsp = require('./live-server-proof') as { runProof(): Promise<import('./types').LiveServerProofRun> };
  ...
  } catch { phases.push({ phase: 'Live server proof', status: 'skipped', detail: 'Module not available' }); }

  const execution: ValidationHarnessExecution = {
    execution_id: uid(), environment: env, phases, proof_run: proofRun,
    created_at: new Date().toISOString(),
  };

  const all = readJson<ValidationHarnessExecution[]>(HARNESS_FILE, []);
...
export function getLatestExecution(): ValidationHarnessExecution | null {
  const all = readJson<ValidationHarnessExecution[]>(HARNESS_FILE, []);
  return all.length > 0 ? all[0] : null;
}

/** Get current environment state (synchronous check from last run) */
export function getEnvironmentState(): ValidationEnvironmentState | null {
