...
exports.getWireStates = getWireStates;
exports.getTruthReport = getTruthReport;
exports.getClosureState = getClosureState;
function uid() { return 'mw_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
/** Get the real wire state of each middleware area — honest about what actually runs */
function getWireStates() {
    const states = [];
    ...
    try {
        ...
        const state = verified > 0 ? 'executed_and_verified' : enforced === routes.length ? 'wired' : enforced > 0 ? 'evaluated_only' : 'design_only';
    ...
    try {
        ...
        const state = verified > 0 ? 'executed_and_verified' : results.length > 0 ? 'wired' : 'evaluated_only';
    ...
    try {
        ...
        const state = verified > 0 ? 'executed_and_verified' : decisions.length > 0 ? 'wired' : 'evaluated_only';
    ...
    try {
        ...
        const state = verified > 0 ? 'executed_and_verified' : 'wired';
    ...
    try {
        ...
        const state = verified > 0 ? 'executed_and_verified' : redacted > 0 ? 'wired' : 'evaluated_only';
...
function getTruthReport() {
    ...
    const truthScore = total > 0 ? Math.round(((executedAndVerified * 100 + wired * 70 + evaluatedOnly * 30 + designOnly * 0) / (total * 100)) * 100) : 0;
    return {
...
//# sourceMappingURL=live-middleware-wiring.js.map