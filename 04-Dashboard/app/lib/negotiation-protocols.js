...
function runNegotiation(sessionId) {
    try {
        ...
        const protocol = getProtocol(session.protocol_type);
        ...
        const outcome = {
            session_id: sessionId, protocol_type: session.protocol_type,
...
function createProtocol(opts) {
    ...
    const p = { ...opts, protocol_id: uid(), created_at: new Date().toISOString() };
    protocols.unshift(p);
    writeJson(PROTOCOLS_FILE, protocols);
    return p;
}
module.exports = { getProtocols, getProtocol, runNegotiation, createProtocol };
//# sourceMappingURL=negotiation-protocols.js.map