...
export function summarizeTaskOutput(taskId: string): { summary: string; keyPoints: string[]; reportCount: number } | null {
  try {
    ...
    if (!allOutput) return { summary: 'No output available', keyPoints: [], reportCount: 0 };
    ...
    const subtaskSummaries = doneSubs.map((s: any) => `${s.title}: ${(s.what_done || '').slice(0, 80)}`).join('; ');
    const summary = `${objective}\n\nCompleted ${doneSubs.length}/${subtasks.length} subtasks. ${subtaskSummaries.slice(0, 300)}`;

    const reportCount = doneSubs.filter((s: any) => s.report_file).length;

    return { summary: summary.slice(0, 500), keyPoints: [...new Set(keyPoints)].slice(0, 10), reportCount };
  } catch { return null; }
}

export function summarizeAllCompleted(): Array<{ taskId: string; title: string; summary: string; keyPointCount: number }> {
  try {
