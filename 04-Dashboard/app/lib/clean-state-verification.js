...
exports.clearValidationState = clearValidationState;
exports.computeFinalGoVerification = computeFinalGoVerification;
const fs = require('fs');
const path = require('path');
const STATE_DIR = path.resolve(__dirname, '..', '..', 'state');
function uid() { return 'csv_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
...
function computeFinalGoVerification() {
    ...
    const productionDecision = computeProductionDecision(networkValidation, reliabilityClosure, cleanState);
    return {
        report_id: uid(),
        production_decision: productionDecision,
        network_validation: networkValidation,
        reliability_closure: reliabilityClosure,
        clean_state: cleanState,
        created_at: new Date().toISOString(),
    };
}
function computeProductionDecision(netVal, relClosure, cleanState) {
    const gates = [];
    const confidence = [];
    const remainingGaps = [];
    ...
    if (netVal) {
        ...
        if (netVal.passed < netVal.total)
            remainingGaps.push(`${netVal.total - netVal.passed} network validation case(s) failed`);
    }
    else {
    ...
    try {
        ...
        if (r.open > 0)
            remainingGaps.push(`${r.open} open blocker(s)`);
    }
    catch {
        gates.push({ gate_id: 'g_blockers', name: 'Blocker reconciliation', required: true, passed: false, detail: 'Module not available' });
    }
    ...
    try {
        ...
        if (wf.partial > 0)
            remainingGaps.push(`${wf.partial} partial workflow(s)`);
    }
    catch {
        gates.push({ gate_id: 'g_workflows', name: 'Workflow closure', required: true, passed: false, detail: 'Module not available' });
    }
    ...
    try {
        ...
        if (t.truth_score < 95)
            remainingGaps.push(`Middleware truth at ${t.truth_score}%`);
    }
    catch {
        gates.push({ gate_id: 'g_middleware', name: 'Middleware truth', required: true, passed: false, detail: 'Module not available' });
    }
    ...
    if (relClosure) {
        ...
        if (relClosure.proxy_only > 0)
            remainingGaps.push(`${relClosure.proxy_only} proxy-only reliability metric(s)`);
    }
    else {
    ...
    if (!cleanState.clean)
        remainingGaps.push(`${cleanState.stale_detected} stale state file(s)`);
    ...
    try {
        ...
        gates.push({ gate_id: 'g_acceptance', name: 'Operator acceptance', required: true, passed: false, detail: 'Module not available' });
    }
    ...
    try {
        ...
        gates.push({ gate_id: 'g_security', name: 'Security posture', required: false, passed: true, detail: 'Assumed adequate' });
    }
    // Decision
    ...
    let decision = 'no_go';
    if (allRequiredPass && overallScore >= 90)
        decision = 'go';
    else if (requiredGates.filter(g => !g.passed).length <= 1 && overallScore >= 75)
...
//# sourceMappingURL=clean-state-verification.js.map