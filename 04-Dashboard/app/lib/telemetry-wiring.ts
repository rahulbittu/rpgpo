...
import type { TelemetryWiringReport, TelemetryWiringState } from './types';

function uid(): string { return 'tw_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }

/** Emit telemetry if observability module is available */
export function emitTelemetry(category: string, action: string, outcome: string, durationMs?: number, metadata?: Record<string, unknown>): void {
  try {
    const obs = require('./observability') as { emit(o: Record<string, unknown>): unknown };
...
export function getWiringReport(): TelemetryWiringReport {
  const flows: Array<{ flow: string; state: TelemetryWiringState; detail: string }> = [
...
/** Get wiring by area */
export function getWiringByArea(area: string): Array<{ flow: string; state: TelemetryWiringState; detail: string }> {
