...
exports.getVisibilityReport = getVisibilityReport;
const fs = require('fs');
...
function validateTask(taskId) {
    ...
    if (!task && !intake)
        return { task_id: taskId, engine_id: 'unknown', closure_state: 'failed_with_reason', deliverable_visible: false, contract_satisfied: false, missing_fields: ['task_not_found'], remediation: 'Task not found', best_partial: null };
    ...
    try {
        ...
        if (!hasAnswer && output?.summary)
            bestPartial = output.summary;
    }
    catch { /* */ }
    // Determine closure state
    let closureState = 'failed_with_reason';
    ...
    if (isDone && hasAnswer)
        closureState = 'final_deliverable_visible';
    else if (isWaiting)
        closureState = 'awaiting_operator_approval';
    else if (isBlocked)
        closureState = 'blocked_with_remediation';
    else if (isDone && !hasAnswer && doneSubs.length > 0)
        closureState = 'blocked_with_remediation';
    else if (isDone && doneSubs.length === 0)
        closureState = 'failed_with_reason';
    // Contract validation
    const missingFields = [];
    if (contract) {
    ...
    if (closureState === 'blocked_with_remediation') {
        ...
        if (!bestPartial && doneSubs.length > 0) {
            ...
            if (best)
                bestPartial = best.output || best.what_done;
        }
    }
...
function getVisibilityReport() {
    ...
    for (const task of doneTasks) {
        ...
        if (status.deliverable_visible)
            visible++;
        else if (status.closure_state === 'blocked_with_remediation') {
            blocked++;
            violations.push({ task_id: task.task_id, engine_id: status.engine_id, issue: status.remediation || 'Blocked' });
        }
        else {
            invisible++;
            violations.push({ task_id: task.task_id, engine_id: status.engine_id, issue: 'No visible deliverable' });
        }
    }
