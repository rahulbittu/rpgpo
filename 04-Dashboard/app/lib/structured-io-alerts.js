...
exports.initStructuredIoAlerts = initStructuredIoAlerts;
...
let _alerts = [];
let _lastEval = {};
function uid() { return 'sio_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
function initStructuredIoAlerts(alertsConfig) {
    if (alertsConfig)
...
function evaluateAlerts(now) {
    ...
    if (snap.totalCalls < cfg.minCalls)
        return [];
    ...
    for (const [key, pm] of Object.entries(snap.byProvider)) {
        if (pm.totalCalls < cfg.minCalls)
            continue;
...
function isOnCooldown(kind, providerKey, now, cooldownMinutes) {
...
//# sourceMappingURL=structured-io-alerts.js.map