...
function uid() { return 'sp_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
...
function runAssessment(scopeType = 'platform', scopeId = 'gpo') {
    const controls = [];
    const findings = [];
    // Tenant isolation
    ...
    if (findings.some(f => f.severity === 'blocker'))
        overall = 'critical';
    else if (findings.filter(f => f.severity === 'high').length >= 2)
        overall = 'weak';
    else if (implemented < total * 0.7)
        overall = 'acceptable';
    const report = {
