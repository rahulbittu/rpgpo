...
export interface ProviderSelection {
  ...
  reason: string;
  confidence: number;
  alternates: Array<{ providerId: string; score: number }>;
}

export function selectProvider(domain: string, taskKind: string): ProviderSelection {
  // Check learning store for historical best
  try {
    const ls = require('./learning-store') as { getBestProvider(key: any): any };
  ...
  const defaults: Record<string, { provider: string; reason: string }> = {
