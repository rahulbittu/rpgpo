...
function readStore(): TaskTemplate[] {
  try { if (fs.existsSync(STORE_FILE)) return JSON.parse(fs.readFileSync(STORE_FILE, 'utf-8')); } catch { /* */ }
  return [];
}

function writeStore(templates: TaskTemplate[]): void {
  const dir = path.dirname(STORE_FILE);
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(STORE_FILE, JSON.stringify(templates, null, 2));
}

export function createTemplate(data: Partial<TaskTemplate>): TaskTemplate {
...
export function listTemplates(filter?: { domain?: string; tags?: string[]; scope?: string }): TaskTemplate[] {
  let templates = readStore();
  if (filter?.domain) templates = templates.filter(t => t.domain === filter.domain);
  if (filter?.tags?.length) templates = templates.filter(t => filter.tags!.some(tag => t.tags.includes(tag)));
  if (filter?.scope) templates = templates.filter(t => t.scope === filter.scope);
...
export function recordTemplateRun(id: string, success: boolean, latencyMs: number, costUsd: number): void {
  const templates = readStore();
...
export function importTemplates(data: TaskTemplate[]): number {
  const templates = readStore();
  let added = 0;
  for (const t of data) {
...
export function exportTemplates(): TaskTemplate[] {
  return readStore();
}
