...
exports.getMemoryViewerData = getMemoryViewerData;
exports.getDocumentsByCategory = getDocumentsByCategory;
exports.getDocumentsByDomain = getDocumentsByDomain;
...
const missionStatements = require('./mission-statements');
...
function collectDecisionDocs() {
    ...
    try {
        if (fs.existsSync(DECISIONS_DIR)) {
            const files = fs.readdirSync(DECISIONS_DIR).filter((f) => f.endsWith('.md')).slice(0, 20);
    ...
    try {
        ...
        for (const f of pr.getAllFits().slice(0, 10)) {
            docs.push({
                ...
                domain: f.scope_level === 'engine' ? f.scope_id : undefined,
...
function collectGovernanceDocs() {
    ...
    try {
        ...
        if (summary.total_hooks > 0)
            docs.push({ id: uid(), category: 'artifact', title: `Runtime: ${summary.total_blocks} blocks, ${summary.active_blocks} active`, summary: `${summary.total_hooks} hooks evaluated`, content: 'Runtime enforcement summary', created_at: summary.created_at });
    ...
    try {
        ...
        if (count > 0)
            docs.push({ id: uid(), category: 'artifact', title: `Artifact Registry: ${count} registered`, summary: 'System artifact registry', content: `${count} artifacts tracked`, created_at: new Date().toISOString() });
    }
    catch { /* */ }
    try {
        ...
        if (count > 0)
            docs.push({ id: uid(), category: 'artifact', title: `Traceability Ledger: ${count} entries`, summary: 'Append-only audit trail', content: `${count} ledger entries`, created_at: new Date().toISOString() });
    }
    catch { /* */ }
    try {
    ...
    try {
        ...
        if (s.total > 0)
            docs.push({ id: uid(), category: 'artifact', title: `Approvals: ${s.pending} pending, ${s.overdue} overdue`, summary: `${s.total} total`, content: 'Approval workspace', created_at: new Date().toISOString() });
    }
    catch { /* */ }
    try {
        ...
        if (items.length > 0)
            docs.push({ id: uid(), category: 'artifact', title: `Escalation Inbox: ${items.length} new`, summary: items[0]?.title || '', content: 'Escalation inbox', created_at: new Date().toISOString() });
    }
    catch { /* */ }
    try {
    ...
    try {
        ...
        if (reports.length > 0)
            docs.push({ id: uid(), category: 'artifact', title: `Security: ${reports[0].overall}`, summary: `${reports[0].findings?.length || 0} findings`, content: 'Security posture report', created_at: reports[0].created_at });
    }
    catch { /* */ }
    try {
    ...
    try {
        ...
        for (const p of sp.getPacks().slice(0, 3))
            docs.push({ id: uid(), category: 'artifact', title: `Skill Pack: ${p.name}`, summary: `v${p.version} (${p.state})`, content: 'Skill pack', created_at: p.created_at });
    }
    catch { /* */ }
    try {
        const et = require('./engine-templates');
        for (const t of et.getTemplates().slice(0, 3))
            docs.push({ id: uid(), category: 'artifact', title: `Template: ${t.name}`, summary: t.domain_type, content: 'Engine template', created_at: t.created_at });
    }
    catch { /* */ }
    try {
        ...
        if (s.total > 0)
            docs.push({ id: uid(), category: 'artifact', title: `Marketplace: ${s.total} listings`, summary: `${s.approved} approved`, content: 'Marketplace summary', created_at: new Date().toISOString() });
    }
    catch { /* */ }
    try {
        const ig = require('./integration-governance');
        for (const i of ig.getIntegrations().slice(0, 3))
            docs.push({ id: uid(), category: 'artifact', title: `Integration: ${i.name}`, summary: `${i.category} | ${i.enabled ? 'enabled' : 'disabled'}`, content: 'Integration connector', created_at: i.created_at });
    }
    catch { /* */ }
    try {
    ...
    try {
        ...
        if (run)
            docs.push({ id: uid(), category: 'artifact', title: `HTTP Validation: ${run.passed}/${run.total} passed`, summary: 'HTTP middleware validation run', content: 'Route-level middleware validation', created_at: run.created_at });
    }
    catch { /* */ }
    try {
    ...
    try {
        ...
        if (run)
            docs.push({ id: uid(), category: 'artifact', title: `Network Validation: ${run.passed}/${run.total} (${run.network_validated} network)`, summary: 'Network-level HTTP validation', content: 'True network-level route validation', created_at: run.created_at });
    }
    catch { /* */ }
    try {
    ...
    try {
        ...
        if (run)
            docs.push({ id: uid(), category: 'artifact', title: `Live Proof: ${run.passed}/${run.total} (${run.live_proven} network)`, summary: run.proof_complete ? 'Fully proven' : 'Partially proven', content: 'Live server proof', created_at: run.created_at });
    }
    catch { /* */ }
    try {
...
function getDocumentsByDomain(domain) {
    const data = getMemoryViewerData();
...
//# sourceMappingURL=memory-viewer.js.map