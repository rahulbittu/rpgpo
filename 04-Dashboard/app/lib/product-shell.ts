...
import type { ProductShellSection, EntryPointDefinition, PrimaryWorkflowStep, ShellClutterIssue, UXConsolidationReport } from './types';

function uid(): string { return 'ps_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }

/** Define all product shell sections with their role and state */
export function getSections(): ProductShellSection[] {
  return [
...
export function getEntryPoints(): EntryPointDefinition[] {
  return [
...
export function getPrimaryWorkflow(): PrimaryWorkflowStep[] {
  return [
...
export function getClutterIssues(): ShellClutterIssue[] {
  return [
...
export function getConsolidationReport(): UXConsolidationReport {
