...
export interface RoutingResult {
  domain: string;          // canonical engine ID
  legacyDomain: string;    // legacy ID for backward compat (will be removed)
  confidence: number;
...
export function routeRequest(rawRequest: string): RoutingResult {
  ...
  for (const [engine, keywords] of Object.entries(ENGINE_KEYWORDS)) {
    ...
    if (score > 0) scores.push({ domain: engine, score });
  }
...
export function toCanonical(domain: string): string {
  return LEGACY_TO_CANONICAL[domain] || domain;
}

/** Convert canonical to legacy (backward compat) */
export function toLegacy(canonical: string): string {
