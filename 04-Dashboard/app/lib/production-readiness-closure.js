...
exports.getShipDecision = getShipDecision;
function uid() { return 'prc_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
...
function computeClosure() {
    ...
    try {
        ...
        const r = aee.getProtectedRoutes();
        aeScore = r.length >= 10 ? 8 : 6;
    }
    catch {
        aeScore = 5;
    }
    ...
    try {
        ...
        mrScore = r.metrics.filter((m) => m.measured).length >= 10 ? 8 : 7;
    }
    catch {
        mrScore = 5;
    }
    ...
    for (const d of dimensions) {
        if (d.status === 'blocked')
            shipBlockers.push(`BLOCKED: ${d.name} — ${d.detail}`);
        else if (d.status === 'partial' && d.score < 6)
            shipBlockers.push(`LOW: ${d.name} — ${d.detail}`);
    }
    const closurePriorities = dimensions.filter(d => d.status !== 'closed').sort((a, b) => a.score - b.score).slice(0, 3).map(d => `${d.name}: ${d.detail}`);
    let shipDecision = 'go';
    if (shipBlockers.length > 0 || overall < 60)
        shipDecision = 'no_go';
    else if (overall < 80)
...
function getShipDecision() {
    ...
    try {
        ...
        deployScore = dr.computeReadiness().overall_score;
    }
    catch { /* */ }
    let secPosture = 'strong';
    try {
        const sh = require('./security-hardening');
        secPosture = sh.runAssessment().overall;
    }
    catch { /* */ }
    let acceptanceRate = 0;
    try {
        const oa = require('./operator-acceptance');
        const r = oa.runAcceptance();
        acceptanceRate = r.checks.length > 0 ? Math.round((r.usable / r.checks.length) * 100) : 0;
    }
    catch { /* */ }
...
//# sourceMappingURL=production-readiness-closure.js.map