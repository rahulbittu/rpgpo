...
export function getTopRankerContracts(): Array<{ id: string; version: string; schema: any }> {
  return ALL_CONTRACTS;
}

export function validateTopRankerPayload(contractId: string, payload: unknown): { valid: boolean; errors?: Array<{ path: string; message: string }> } {
  const contract = ALL_CONTRACTS.find(c => c.id === contractId);
  if (!contract) return { valid: false, errors: [{ path: '', message: `Unknown contract: ${contractId}` }] };
  ...
  for (const [field, def] of Object.entries(schema.properties || {}) as Array<[string, any]>) {
    if (def.type === 'array' && field in (payload as Record<string, unknown>)) {
