...
import type { OperatorAcceptanceRun } from './types';

function uid(): string { return 'oa_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }

/** Run operator acceptance checks */
export function runAcceptance(): OperatorAcceptanceRun {
  const checks: Array<{ name: string; status: OperatorAcceptanceRun['checks'][0]['status']; detail: string }> = [
    // Approvals
