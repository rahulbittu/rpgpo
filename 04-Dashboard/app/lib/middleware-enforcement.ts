...
import type { MiddlewareEnforcementResult } from './types';

/** Evaluate a request through middleware enforcement */
export function enforce(route: string, tenantId: string = 'rpgpo'): { allowed: boolean; reason: string } {
  // Check entitlement
  try {
    ...
    if (decision.outcome.startsWith('denied')) {
      // Emit telemetry
      try { const tw = require('./telemetry-wiring') as { emitTelemetry(c: string, a: string, o: string): void }; tw.emitTelemetry('middleware', 'entitlement_denied', 'blocked'); } catch { /* */ }
...
export function getCoverageReport(): MiddlewareEnforcementResult[] {
  // Delegate to live-middleware-wiring for honest state
  try {
    const lmw = require('./live-middleware-wiring') as { getWireStates(): Array<{ area: string; state: string; route_count: number; enforced_count: number; detail: string }> };
    const states = lmw.getWireStates();
    return states.map(s => {
      let meState: MiddlewareEnforcementResult['state'] = 'missing';
      if (s.state === 'executed_and_verified') meState = 'enforced';
      else if (s.state === 'wired') meState = 'enforced';
      else if (s.state === 'evaluated_only') meState = 'partially_enforced';
      else meState = 'missing';
      return { area: s.area, route_count: s.route_count, enforced_count: s.enforced_count, state: meState, detail: s.detail };
    });
  } catch { /* fall through to legacy */ }

  const results: MiddlewareEnforcementResult[] = [];
  try {
  ...
  } catch { results.push({ area: 'api_entitlements', route_count: 0, enforced_count: 0, state: 'missing', detail: 'Module not available' }); }
