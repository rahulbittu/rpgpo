...
function readScores(): QualityScore[] {
  try { if (fs.existsSync(SCORES_FILE)) return JSON.parse(fs.readFileSync(SCORES_FILE, 'utf-8')); } catch { /* */ }
  return [];
}

function writeScores(scores: QualityScore[]): void {
  if (scores.length > 500) scores = scores.slice(-500);
  const dir = path.dirname(SCORES_FILE);
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(SCORES_FILE, JSON.stringify(scores, null, 2));
}

export function scoreTask(taskId: string): QualityScore {
  try {
    ...
    const allOutput = doneSubs.map((s: any) => (s.output || s.what_done || '')).join('\n');
    ...
    try {
      const ls = require('./learning-store') as { recordProviderSample(key: any, sample: any): void };
      for (const sub of doneSubs) {
        ls.recordProviderSample(
          { engineId: task.domain || 'general', taskKind: sub.stage || 'subtask', contractName: sub.assigned_role || 'general' },
          { timestamp: Date.now(), providerId: sub.assigned_model || 'unknown', latencyMs: 0, inputTokens: 0, outputTokens: 0, totalCostUsd: 0, success: true, qualityScore: overall }
        );
      }
    } catch { /* non-fatal */ }
...
export function rateTask(taskId: string, rating: number): boolean {
  const scores = readScores();
...
export function getScore(taskId: string): QualityScore | null {
  return readScores().find(s => s.taskId === taskId) || null;
}

export function getScores(limit?: number): QualityScore[] {
  return readScores().sort((a, b) => b.timestamp - a.timestamp).slice(0, limit || 50);
}

export function getAverageQuality(): { overall: number; dimensions: Record<string, number>; count: number } {
  const scores = readScores();
  if (scores.length === 0) return { overall: 0, dimensions: {}, count: 0 };
...
function scoreCompleteness(task: any, doneSubs: any[], allSubs: any[]): number {
