...
export function generateMorningDigest(): string {
  ...
  try {
    ...
    const recentDone = tasks.filter((t: any) => t.status === 'done' && new Date(t.updated_at || '').getTime() > yesterday);
    const active = tasks.filter((t: any) => !['done', 'failed', 'canceled'].includes(t.status));
    digest += `## Yesterday's Results\n- ${recentDone.length} tasks completed\n`;
    recentDone.slice(0, 5).forEach((t: any) => { digest += `- ${(t.title || t.raw_request || '').slice(0, 60)}\n`; });
    digest += `\n## Active Today\n- ${active.length} tasks in progress\n`;
    active.forEach((t: any) => { digest += `- [${t.status}] ${(t.title || '').slice(0, 50)}\n`; });
  } catch { digest += 'Could not load task data\n'; }
  try {
    const costs = require('./costs') as { getSummary(): any };
    const s = costs.getSummary();
    digest += `\n## Costs\n- Yesterday: $${(s.today?.cost || 0).toFixed(4)} (${s.today?.calls || 0} calls)\n- This week: $${(s.week?.cost || 0).toFixed(4)}\n`;
  } catch { /* */ }
  try {
    const se = require('./suggestions-engine') as { getSuggestions(n: number): any[] };
    const suggestions = se.getSuggestions(3);
    digest += `\n## Suggested Tasks\n`;
    suggestions.forEach((s: any) => { digest += `- **${s.title}**: ${s.description}\n`; });
  } catch { /* */ }
  return digest;
}

export function generateEveningWrap(): string {
  let wrap = `# GPO Evening Wrap\n**${new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}**\n\n`;
  try {
    const intake = require('./intake') as { getAllTasks(): any[] };
    const today = new Date().toISOString().slice(0, 10);
    const todayTasks = intake.getAllTasks().filter((t: any) => (t.created_at || '').startsWith(today));
    const done = todayTasks.filter((t: any) => t.status === 'done');
    wrap += `## Today's Summary\n- ${todayTasks.length} tasks submitted\n- ${done.length} completed\n`;
    done.forEach((t: any) => { wrap += `- ${(t.title || t.raw_request || '').slice(0, 60)}\n`; });
  } catch { /* */ }
  try {
