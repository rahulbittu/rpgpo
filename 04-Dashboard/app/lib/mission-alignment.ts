...
export function checkAlignment(rawRequest: string, domain: string): AlignmentResult {
  const lower = rawRequest.toLowerCase();
  const matchedPriorities: string[] = [];
  const matchedMissions: string[] = [];
  ...
  try {
    ...
    if (fs.existsSync(profileFile)) {
      ...
      for (const priority of (profile.recurring_priorities || [])) {
        const keywords = priority.toLowerCase().split(/[,;:]+/).map((s: string) => s.trim()).filter((s: string) => s.length > 3);
        for (const kw of keywords) {
          if (lower.includes(kw.slice(0, 15))) { matchedPriorities.push(priority); break; }
        }
      }
    }
  } catch { /* */ }
  ...
  try {
    ...
    if (fs.existsSync(msFile)) {
      ...
      for (const ms of statements) {
        ...
        const objectives = (ms.objectives || []).map((o: string) => o.toLowerCase());
        const allText = [msText, ...objectives].join(' ');
        const words = lower.split(/\s+/).filter((w: string) => w.length > 4);
        const matches = words.filter((w: string) => allText.includes(w)).length;
        if (matches >= 2) matchedMissions.push(ms.scope_id || ms.id || 'unknown');
      }
    }
  } catch { /* */ }
