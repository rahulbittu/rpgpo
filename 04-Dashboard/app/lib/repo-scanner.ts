...
import type { Domain, RepoGrounding, RepoCandidate, TargetArea, PathValidation } from './types';
...
const PROJECT_ROOTS: Partial<Record<Domain, string>> = {
...
const CODE_EXTS: Set<string> = new Set(['.ts', '.tsx', '.js', '.jsx', '.json', '.sql', '.css', '.md']);
const SKIP_DIRS: Set<string> = new Set(['node_modules', '.git', 'dist', '.expo', '.next', 'android', 'ios', 'screenshots', 'attached_assets', 'assets', 'patches']);

/** Extended RepoGrounding with extra fields used internally */
interface FullRepoGrounding extends RepoGrounding {
  allFiles?: string[];
}
 ...
 * Returns { dirs: [...], files: [...] }
 */
function scanDir(absRoot: string, maxDepth: number = 3): { dirs: string[]; files: string[] } {
  const dirs: string[] = [];
  const files: string[] = [];

  function walk(dir: string, depth: number, relPath: string): void {
    ...
    for (const e of entries) {
      ...
      const rel: string = relPath ? relPath + '/' + e.name : e.name;
      ...
      if (e.isDirectory()) {
        ...
        const ext: string = path.extname(e.name).toLowerCase();
 ...
 */
function buildStructuralMap(domain: Domain): { tree: string; files: string[]; dirs: string[]; projectRelRoot: string } | null {
  const projectRelRoot = PROJECT_ROOTS[domain];
  if (!projectRelRoot) return null;

  const absRoot: string = path.join(RPGPO_ROOT, projectRelRoot);
 ...
 */
function inferTargetAreas(taskText: string, domain: Domain): TargetArea[] {
 ...
 */
function findCandidateFiles(domain: Domain, targetAreas: TargetArea[], allFiles: string[], projectRelRoot: string): RepoCandidate[] {
  ...
  for (const area of targetAreas) {
    for (const dir of area.dirs) {
      ...
      for (const m of matching) {
        ...
        const fullPath: string = path.join(RPGPO_ROOT, repoRelPath);
 ...
 */
function groundInRepo(domain: Domain, taskText: string): FullRepoGrounding {
 ...
 */
function validatePaths(filePaths: string[]): PathValidation {
  const missing: string[] = [];
  for (const f of filePaths) {
    const fullPath: string = path.join(RPGPO_ROOT, f);
    if (!fs.existsSync(fullPath)) {
      missing.push(f);
    }
  }
