...
import type {
  SimulationScope, ReleaseReadinessScore, ReadinessRecommendation, ReadinessRule, Lane,
} from './types';
...
function defaultRules(): ReadinessRule[] {
  return [
...
export function getRules(): ReadinessRule[] {
  const stored = readJson<ReadinessRule[]>(RULES_FILE, []);
...
export function computeScore(relatedType: SimulationScope, relatedId: string): ReleaseReadinessScore {
  const rules = getRules().filter(r => r.enabled);
  const categories: Record<string, { score: number; max: number; details: string }> = {};
  ...
  if (relatedType === 'dossier') {
    try {
      ...
      if (d) { lane = d.lane; graphId = d.graph_id; }
    } catch { /* */ }
    ...
    switch (rule.check_fn) {
      case 'checkPolicySim': {
        try {
          const sim = require('./policy-simulation') as {
            runSimulation(rt: string, rid: string, l: string): import('./types').SimulationResult;
          };
          const r = sim.runSimulation(relatedType, relatedId, lane);
          if (r.outcome === 'pass') { score = rule.weight; detail = 'Policy simulation passed'; }
          else if (r.outcome === 'warn') { score = Math.round(rule.weight * 0.6); detail = `Warnings: ${r.warnings.join(', ')}`; warnings.push(...r.warnings); }
          else { score = 0; detail = `Blocked: ${r.blocked_actions.join(', ')}`; blockers.push(...r.blocked_actions); }
        } catch { score = Math.round(rule.weight * 0.5); detail = 'Simulation unavailable'; }
        break;
      }
      case 'checkReviews': {
        try {
          ...
          if (failed > 0) { blockers.push(`${failed} review(s) failed`); detail += `, ${failed} failed`; }
          if (reviews.some(r => !r.verdict)) warnings.push('Some reviews pending');
        } catch { score = 0; detail = 'Reviews unavailable'; }
        break;
      }
      case 'checkDocs': {
        try {
          ...
          if (check.met) { score = rule.weight; detail = 'All docs present'; }
          else { score = 0; detail = `Missing: ${check.missing.join(', ')}`; if (check.blocking) blockers.push(`Docs missing: ${check.missing.join(', ')}`); else warnings.push(`Docs missing: ${check.missing.join(', ')}`); }
        } catch { score = Math.round(rule.weight * 0.5); detail = 'Doc check unavailable'; }
        break;
      }
      case 'checkEscalation': {
        try {
          ...
          if (unresolved.length === 0) { score = rule.weight; detail = 'No unresolved escalations'; }
          else { score = 0; detail = `${unresolved.length} unresolved escalation(s)`; warnings.push(`${unresolved.length} unresolved escalation(s)`); }
        } catch { score = rule.weight; detail = 'No escalation data'; }
        break;
      }
      case 'checkProviders': {
        try {
          const pr = require('./provider-registry') as { getAllFits(): import('./types').ScopedProviderFit[] };
          const fits = pr.getAllFits().filter((f: import('./types').ScopedProviderFit) => f.state !== 'deprecated');
          const avgConf = fits.length > 0 ? fits.reduce((s: number, f: import('./types').ScopedProviderFit) => s + f.confidence, 0) / fits.length : 50;
          score = Math.round((Math.min(avgConf, 100) / 100) * rule.weight);
          detail = `Avg provider confidence: ${Math.round(avgConf)}%`;
          if (avgConf < 30) warnings.push('Low provider confidence');
        } catch { score = Math.round(rule.weight * 0.5); detail = 'Provider data unavailable'; }
        break;
      }
      case 'checkMission': {
        try {
          const ms = require('./mission-statements') as {
            checkMissionAlignment(desc: string, d?: string): { aligned: boolean; reason: string };
          };
          ...
          if (alignment.aligned) { score = rule.weight; detail = alignment.reason; }
          else { score = Math.round(rule.weight * 0.3); detail = alignment.reason; warnings.push('Mission alignment concern'); }
        } catch { score = Math.round(rule.weight * 0.5); detail = 'Mission check unavailable'; }
        break;
      }
      case 'checkRisks': {
        try {
          const pd = require('./promotion-dossiers') as { getDossierForGraph(id: string): import('./types').PromotionDossier | null };
          const dossier = pd.getDossierForGraph(graphId);
          if (dossier) {
            const unresolvedRisks = dossier.risks.length + dossier.unresolved_items.length;
            if (unresolvedRisks === 0) { score = rule.weight; detail = 'All risks resolved'; }
            else { score = Math.max(0, rule.weight - unresolvedRisks * 3); detail = `${unresolvedRisks} unresolved risk(s)/item(s)`; warnings.push(`${unresolvedRisks} unresolved risk(s)`); }
          } else { score = Math.round(rule.weight * 0.5); detail = 'No dossier available'; }
        } catch { score = Math.round(rule.weight * 0.5); detail = 'Risk check unavailable'; }
        break;
      }
    }
  ...
  let recommendation: ReadinessRecommendation = 'ready';
  if (blockers.length > 0 || pct < 40) recommendation = 'not_ready';
  else if (warnings.length > 0 || pct < 75) recommendation = 'conditional';

  const scoreRecord: ReleaseReadinessScore = {
    score_id: uid(),
  ...
  const scores = readJson<ReleaseReadinessScore[]>(SCORES_FILE, []);
...
export function getScoresForEntity(relatedType: SimulationScope, relatedId: string): ReleaseReadinessScore[] {
  return readJson<ReleaseReadinessScore[]>(SCORES_FILE, []).filter(s => s.related_type === relatedType && s.related_id === relatedId);
}

export function getAllScores(): ReleaseReadinessScore[] {
  return readJson<ReleaseReadinessScore[]>(SCORES_FILE, []);
}
