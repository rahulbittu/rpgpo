...
exports.generateDocsHtml = generateDocsHtml;
const fs = require('fs');
...
function categorize(path) {
    if (path.includes('/intake'))
        return 'Tasks & Intake';
    if (path.includes('/chief-of-staff'))
        return 'Chief of Staff';
    if (path.includes('/topranker'))
        return 'TopRanker';
    ...
    if (path.includes('/compound'))
        return 'Compound Workflows';
    if (path.includes('/conversation'))
        return 'Conversations';
    if (path.includes('/chain'))
        return 'Task Chaining';
    if (path.includes('/backup'))
        return 'Backup';
    if (path.includes('/integration'))
        return 'Integrations';
    if (path.includes('/health') || path.includes('/onboarding'))
        return 'Health & Setup';
    ...
    if (path.includes('/structured-io'))
        return 'Structured I/O';
    if (path.includes('/provider'))
        return 'Providers';
    if (path.includes('/ai-io'))
        return 'AI I/O';
