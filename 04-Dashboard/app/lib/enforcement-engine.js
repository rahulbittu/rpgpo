...
exports.getDecisionsForEntity = getDecisionsForEntity;
...
function uid() { return 'ed_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
...
function evaluate(relatedType, relatedId, action, lane) {
    const reasons = [];
    const blockers = [];
    const warnings = [];
    ...
    for (const rule of rules) {
        ...
        switch (rule.condition) {
            case 'documentation_incomplete': {
                try {
                    ...
                    if (!check.met) {
                        triggered = true;
                        reasons.push(`Missing docs: ${check.missing.join(', ')}`);
                        requiredOverrides.push('documentation_gap');
                    }
                }
                catch { /* */ }
                break;
            ...
            case 'reviews_not_passed': {
                try {
                    ...
                    if (graphId) {
                        ...
                        if (failed.length > 0) {
                            triggered = true;
                            reasons.push(`${failed.length} review(s) failed`);
                            requiredOverrides.push('review_failure');
                        }
                        const pending = reviews.filter(r => !r.verdict);
                        if (pending.length > 0) {
                            triggered = true;
                            reasons.push(`${pending.length} review(s) pending`);
                        }
                    }
                }
                catch { /* */ }
            ...
            case 'readiness_below_50': {
                ...
                try {
                    ...
                    if (latest && latest.overall_score < threshold) {
                        triggered = true;
                        reasons.push(`Readiness ${latest.overall_score}% < ${threshold}%`);
                        requiredOverrides.push('readiness_shortfall');
                    }
                    else if (!latest) {
                        triggered = true;
                        reasons.push('No readiness score computed');
                        requiredOverrides.push('readiness_shortfall');
                    }
                }
                catch { /* */ }
                ...
                try {
                    ...
                    if (graphId) {
                        ...
                        if (unresolved.length > 0) {
                            triggered = true;
                            reasons.push(`${unresolved.length} unresolved escalation(s)`);
                            requiredOverrides.push('escalation_conflict');
                        }
                    }
                }
                catch { /* */ }
            ...
            case 'graph_incomplete': {
                try {
                    const egMod = require('./execution-graph');
                    const graphId = getGraphId(relatedType, relatedId);
                    if (graphId) {
                        const check = egMod.isGraphComplete(graphId);
                        if (!check.complete) {
                            triggered = true;
                            reasons.push('Graph not yet complete');
                        }
                    }
                }
                catch { /* */ }
            ...
            case 'not_promoted': {
                try {
                    ...
                    if (!dossier || !dossier.promoted_at) {
                        triggered = true;
                        reasons.push('Not yet promoted via dossier');
                        requiredOverrides.push('promotion_block');
                    }
                }
                catch { /* */ }
                break;
        ...
        if (triggered) {
            ...
            else if (rule.level === 'soft_block') {
                blockers.push(rule.description);
                if (level === 'allow' || level === 'warn')
                    level = 'soft_block';
            }
            else if (rule.level === 'override_required') {
                blockers.push(rule.description);
                if (level === 'allow' || level === 'warn')
                    level = 'override_required';
            }
...
function getGraphId(relatedType, relatedId) {
    if (relatedType === 'graph')
        return relatedId;
...
function getDecisionsForEntity(relatedType, relatedId) {
    return readJson(DECISIONS_FILE, []).filter(d => d.related_type === relatedType && d.related_id === relatedId);
}
function getDecision(decisionId) {
    return readJson(DECISIONS_FILE, []).find(d => d.decision_id === decisionId) || null;
}
module.exports = {
    getRules, evaluate,
