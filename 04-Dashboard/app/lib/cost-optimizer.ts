...
export function getCostInsights(): CostInsight[] {
  ...
  const expensiveCalls = recentCosts.filter((c: any) => (c.cost || c.totalCost || 0) > avgCostPerCall * 3);
  ...
  for (const c of costs) {
    ...
    if (day && day !== 'Inva') dayTotals[day] = (dayTotals[day] || 0) + (c.cost || c.totalCost || 0);
  }
...
function readJson(f: string, fb: any): any {
