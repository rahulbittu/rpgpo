...
exports.getProtectedPaths = getProtectedPaths;
...
function validatePath(pathId) {
    ...
    if (pp.protection_type === 'boundary' || pp.protection_type === 'redaction') {
        try {
            ...
            const observed = result.outcome === 'blocked' ? 'deny' : result.outcome === 'redacted' ? 'redact' : 'allow';
            steps.push({
    ...
    if (pp.protection_type === 'tenant_isolation') {
        ...
        try {
            ...
            const observed = result.allowed ? 'allow' : 'deny';
            steps.push({
    ...
    if (pp.protection_type === 'extension_permission') {
        try {
            ...
            const observed = result.outcome.startsWith('denied') ? 'deny' : 'allow';
            steps.push({
    ...
    if (pp.protection_type === 'provider_gate') {
        try {
            ...
            const observed = result.outcome === 'blocked' ? 'deny' : 'allow';
            steps.push({
...
function getSummary() {
    ...
    for (const run of runs) {
        if (!latestByPath[run.path_id])
            latestByPath[run.path_id] = run;
    }
...
//# sourceMappingURL=protected-path-validation.js.map