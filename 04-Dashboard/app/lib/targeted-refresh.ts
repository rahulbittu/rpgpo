...
import type { TargetedRefreshPlan } from './types';

/** Define targeted refresh plans per area */
export function getPlans(): TargetedRefreshPlan[] {
  return [
...
export function getPlanForArea(area: string): TargetedRefreshPlan | null {
  return getPlans().find(p => p.area === area) || null;
}

/** Generate JS code for targeted refresh */
export function getRefreshCode(area: string): string {
  const plan = getPlanForArea(area);
  if (!plan) return 'location.reload()';
