...
exports.getAuditByArea = getAuditByArea;
function uid() { return 'ua_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
/** Define all known UI surfaces and their wiring state */
function defineSurfaces() {
    return [
        // Home / CoS
...
function runAudit() {
    ...
    const topGaps = [];
    if (apiOnly > 0)
        topGaps.push(`${apiOnly} surfaces are API-only with no UI rendering`);
    if (partial > 0)
        topGaps.push(`${partial} surfaces are partially wired`);
