...
export type TaskStatus = 'intake' | 'deliberating' | 'planned' | 'executing' | 'waiting_approval' | 'waiting_human' | 'blocked' | 'done' | 'failed' | 'canceled';
/** Canonical subtask lifecycle states */
export type SubtaskStatus = 'proposed' | 'queued' | 'running' | 'builder_running' | 'waiting_approval' | 'builder_fallback' | 'waiting_human' | 'blocked' | 'done' | 'failed' | 'canceled';
...
export type Domain = 'topranker' | 'careeregine' | 'founder2founder' | 'wealthresearch' | 'newsroom' | 'screenwriting' | 'music' | 'personalops' | 'general';
/** AI provider identifiers */
export type Provider = 'claude' | 'openai' | 'perplexity' | 'gemini';
/** Models available per provider */
export type ModelId = 'claude' | 'gpt-4o' | 'gpt-4o-mini' | 'sonar' | 'sonar-pro' | 'gemini-2.5-flash-lite' | 'gemini-2.5-flash' | string;
...
export type BuilderOutcome = 'code_applied' | 'no_changes' | 'builder_fallback_prompt_created' | 'builder_timeout' | 'blocked_missing_context' | 'manual_execution_confirmed' | 'code_applied_approved' | 'no_changes_approved' | 'rejected' | null;
/** Builder execution phases */
export type BuilderPhase = 'inspecting' | 'launching' | 'preflight' | 'running' | 'diffing' | 'classifying' | 'reporting' | 'review' | 'fallback' | 'complete' | 'blocked' | null;
/** Subtask stages from deliberation */
export type SubtaskStage = 'audit' | 'decide' | 'implement' | 'build' | 'code' | 'locate_files' | 'report' | 'research' | 'review' | 'strategy' | 'approve' | string;
/** Subtask outcome classification */
export type OutcomeType = 'code_applied' | 'files_written' | 'text_only_build' | 'text_output' | 'blocked_missing_context' | null;
/** Queue task status */
export type QueueTaskStatus = 'queued' | 'running' | 'done' | 'failed';
/** Queue task types */
export type QueueTaskType = 'refresh-state' | 'morning-loop' | 'evening-loop' | 'board-run' | 'deliberate' | 'execute-subtask' | 'execute-builder' | 'launch_builder' | 'ai-channel' | string;
...
export interface GPOInstance {
    ...
    repo_mappings: Record<Domain, string>;
...
export type BlockerReason = 'approval_required' | 'code_review_required' | 'builder_fallback' | 'revision_requested' | 'missing_context' | 'budget_exceeded' | 'provider_unavailable' | 'privacy_restricted' | 'policy_blocked' | 'rate_limited' | 'manual_action_required';
...
export type NotificationEventType = 'approval_needed' | 'task_done' | 'task_failed' | 'builder_complete' | 'builder_fallback' | 'budget_warning' | 'budget_exceeded' | 'provider_down' | 'loop_blocked' | 'loop_degraded';
...
export type BoardLifecyclePhase = 'interpret' | 'research' | 'critique' | 'synthesize' | 'decide' | 'handoff' | 'review' | 'report';
...
export type InceptionRoute = 'direct_answer' | 'board_deliberation' | 'research_heavy' | 'builder_heavy' | 'review_report' | 'clarification_needed' | 'blocked';
...
export interface OperatorBehavioralProfile {
    ...
    value_priorities: Array<{
        priority: string;
        weight: number;
    }>;
    correction_tendencies: Array<{
        tendency: string;
        frequency: number;
    }>;
    detail_expectations: {
        default_level: 'minimal' | 'standard' | 'detailed';
        varies_by_domain: Record<string, string>;
    };
    updated_at: string;
}
...
export type PolicyArea = 'execution_style' | 'review_strictness' | 'documentation_strictness' | 'provider_override_mode' | 'interruption_mode' | 'learning_promotion_mode' | 'board_recheck_bias';
...
export type EscalationTrigger = 'low_confidence' | 'review_conflict' | 'handoff_quality' | 'privacy_risk' | 'mission_conflict' | 'documentation_gap' | 'provider_mismatch' | 'retry_exhaustion' | 'promotion_attempt';
/** Actions the system takes on escalation */
export type EscalationAction = 'notify_operator' | 'require_operator_approval' | 'board_reopen' | 'require_second_provider_review' | 'pause_execution' | 'downgrade_to_advisory';
/** An escalation rule */
...
export interface OperatorAcceptanceRun {
    ...
    checks: Array<{
        name: string;
        status: 'usable' | 'partially_usable' | 'broken' | 'blocked_backend' | 'blocked_ui';
        detail: string;
    }>;
    usable: number;
    partial: number;
...
export interface CreativeDraftDeliverable {
    ...
    generatedAt: string;
    artifacts: Array<{
        type: 'poem' | 'script' | 'lyrics' | 'story' | 'beat_sheet' | 'premise';
        content: string;
    }>;
}
...
export interface DeliverableRendererDef {
    key: RendererKey;
    render(model: RenderModel, opts?: {
        compact?: boolean;
    }): string;
}
...
export interface TopRankerCategory {
    id: string;
    name: string;
    slug: string;
}
...
export interface TopRankerReviewAggregation {
    ...
    sources: Array<{
        source: 'google' | 'yelp' | 'facebook' | 'opentable' | 'other';
        count: number;
        avgRating: number;
        lastReviewAt?: string;
    }>;
    ...
    aggregationMethod: 'bayesian' | 'wilson' | 'hybrid';
    computedAt: string;
}
...
export type NotificationType = 'approval.requested' | 'alert.fired' | 'provider.circuit.open' | 'provider.circuit.closed' | 'workflow.stuck' | 'workflow.failed' | 'deliverable.pending-approval' | 'release.ready' | 'system.info';
...
export interface ChainEvaluationResult {
    triggered: boolean;
    tasksToCreate: Array<{
        template: ChainRule['nextTaskTemplate'];
        ruleId: string;
        parentOutputRef?: string;
    }>;
}
export interface CompoundWorkflowTemplate {
    ...
    name: string;
    description: string;
    nodes: CompoundWorkflowNode[];
    edges: CompoundWorkflowEdge[];
...
export interface CompoundWorkflowNode {
    ...
    inputBindings?: Record<string, {
        source: 'const' | 'context' | 'nodeOutput' | 'param';
        nodeId?: string;
        path?: string;
        value?: unknown;
        paramName?: string;
...
export interface CompoundWorkflowEdge {
    from: string;
    to: string;
    condition?: string;
}
export interface CompoundWorkflowParam {
    name: string;
    type: string;
    required: boolean;
    default?: unknown;
